...
async def balance(msg):
    if not(valid_currency(msg)):
        logs.printWarning("Module currency not enabled.")
...
async def daily(msg):
    ...
    db = sqlite3.connect("discordRem.db")
    cursor = db.cursor()
    cursor.execute("SELECT money FROM Users WHERE user_id=?", (msg.author.id,))
    value = cursor.fetchone()
    if value is None:
        await msg.channel.send("Vous n'êtes pas dans la base de donnée, pour y être faites `!init`.")
    else:
        cursor.execute("UPDATE Users SET money=? WHERE user_id=?", (value[0] + 200, msg.author.id))
        embed = discord.Embed(title="Gain journaliers:", color=0xe6ca03, description=":coin: Vous avez gagné 200¢")
        await msg.channel.send(embed=embed)
    db.close()
